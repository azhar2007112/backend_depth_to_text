...
def analyze_depth_image(depth_image: Image.Image) -> str:
    try:
        ...
        if max(depth_image.size) > max_size:
            ...
            logging.info(f"Image resized to {depth_image.size} for efficient processing.")
        ...
        human_message = HumanMessage(content=[ 
            ...
            {"type": "text", "text": "Provide a one-line analysis for blind navigation."}
        ])
        ...
        logging.error(f"Error analyzing depth map: {str(e)}")
        return f"Error analyzing depth map: {str(e)}"
