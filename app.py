...
def analyze_depth_in_background(depth_image):
    # Run the analyze function in a separate thread
    analysis = analyze_depth_image(depth_image)
    print(f"Depth analysis completed in background.")
...
def generate_depth():
    ...
    try:
        ...
        img_io.seek(0)
        print(f"Depth image saving time: {time.time() - step_start:.4f} seconds")

        # Run analysis in background thread
        analysis_thread = threading.Thread(target=analyze_depth_in_background, args=(depth_image,))
        analysis_thread.start()  # Start analysis in the background
        print(f"Depth analysis started in background: {time.time() - step_start:.4f} seconds")
